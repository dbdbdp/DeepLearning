    ...
    def fit(self, xs, ts, max_epoch=10, batch_size=20, time_size=35,
        ...
        for epoch in range(max_epoch):
            for iters in range(max_iters):
                ...
                # 기울기를 구해 매개변수 갱신
                loss = model.forward(batch_x, batch_t)                       # 모든 타임 시퀀스에 대해 더한다.
                model.backward()
                params, grads = remove_duplicate(model.params, model.grads)  # 공유된 가중치를 하나로 모음(더한다고 보면 됨.)(모든 배치들의 기울기를 합산한다.)
                if max_grad is not None:
                    clip_grads(grads, max_grad)
                optimizer.update(params, grads)
                total_loss += loss
